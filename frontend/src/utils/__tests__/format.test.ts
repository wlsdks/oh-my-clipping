...
import { toInt, toFloat, currentYearMonth, normalizeLegacyBundleTerm } from "@/utils/format";
...
describe("normalizeLegacyBundleTerm", () => {
  it("replaces 요약본 with 뉴스 모음", () => {
    expect(normalizeLegacyBundleTerm("오늘의 요약본입니다")).toBe("오늘의 뉴스 모음입니다");
  });
  it("no match returns original string", () => {
    expect(normalizeLegacyBundleTerm("nothing here")).toBe("nothing here");
  });
});
