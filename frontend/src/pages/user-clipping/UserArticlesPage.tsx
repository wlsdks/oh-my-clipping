...
import { UndeliveredDigestBanner } from "./UndeliveredDigestBanner";
...
function groupByDate(items: ArticleHistoryItem[]): { date: string; label: string; items: ArticleHistoryItem[] }[] {
...
type ArticleTab = "my-news" | "competitor-news";


export function UserArticlesPage() {
  const qc = useQueryClient();
  const [activeTab, setActiveTab] = useState<ArticleTab>("my-news");
  ...
  // Build category list from user's own approved requests only
  const userCategories: UserCategory[] = requests
    .filter((r) => r.status === "APPROVED" && r.approvedCategoryId && r.approvedCategoryName)
  ...
  const isLoading = isLoadingRequests || isLoadingHistory;
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5">
      <div>
        <h1 className="text-2xl font-bold">내 기사 목록</h1>
        <p className="text-sm text-muted-foreground mt-1">
          받아본 기사를 검색하고, 중요한 기사를 저장해 모아볼 수 있어요.
        </p>
      </div>

      <UndeliveredDigestBanner />

      {/* 구독 뉴스 / 경쟁사 뉴스 서브탭 */}
      <div className="flex border-b">
        {[
          { key: "my-news" as const, label: "구독 뉴스" },
          { key: "competitor-news" as const, label: "경쟁사 뉴스" }
        ].map((tab) => (
          <button
            key={tab.key}
            type="button"
