...
tasks.register("checkApiModelBoundaries") {
    group = "verification"
    description = "Ensure API model module stays free of Spring/JPA/store/app model dependencies."
    ...
    doLast {
            ...
            .forEach { file ->
                ...
                forbiddenImports.forEach { (pattern, label) ->
                    pattern.findAll(source).forEach { match ->
                        ...
                        violations += "${file.relativeTo(sourceRoot).path}:$line — forbidden API model dependency ($label)"
                    }
                }
            }
        ...
        if (violations.isNotEmpty()) {
            ...
            throw GradleException(
                "${violations.size} API model boundary violation(s) detected. " +
                    "Keep API models free of Spring/JPA/store/app model dependencies.",
            )
        }

        logger.lifecycle("checkApiModelBoundaries: OK (API models have no forbidden app/framework imports)")
    }
}

tasks.named("check") {
    dependsOn("checkApiModelBoundaries")
}
