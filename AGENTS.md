...
- prepared digest 생성/발송/finalization 경계는 `ports/workflow`의 `service/port/DigestDeliveryWorkflowPort`에 둔다. 포트는 엔진 파이프라인 DTO와 동일한 `PipelineDigestResult`/`PipelineDigestItemResult`를 노출한다 (DTO 는 `core/api-models`가 소유하며 엔진 모듈과 공유). `SlackDigestWorker`는 스케줄/예약/retry orchestration만 담당하고, 기존 `ClippingService` 호출은 `DigestDeliveryWorkflowAdapter` 내부로 제한한다.
