...
class UrlSafetyValidator {
    ...
    fun validatePublicHttpUrl(rawUrl: String): URI {
        val uri = try {
            ...
            throw InvalidInputException("올바르지 않은 URL 형식입니다.")
        }

        val scheme = uri.scheme?.lowercase()
            ?: throw InvalidInputException("http:// 또는 https://로 시작하는 주소를 입력해 주세요.")
        if (scheme != "http" && scheme != "https") {
            throw InvalidInputException("http:// 또는 https://로 시작하는 주소만 허용됩니다.")
        }

        val host = uri.host ?: throw InvalidInputException("URL에 호스트가 필요합니다.")
        val normalizedHost = normalizeHost(host)
        if (normalizedHost == "localhost") {
            throw InvalidInputException("localhost 주소는 허용되지 않습니다.")
        }

        val addresses = try {
            InetAddress.getAllByName(normalizedHost)
        } catch (_: UnknownHostException) {
            throw InvalidInputException("URL 호스트를 확인할 수 없습니다. 주소를 다시 확인해 주세요.")
        }

        if (addresses.isEmpty()) {
            throw InvalidInputException("URL 호스트를 확인할 수 없습니다.")
        }
        if (addresses.any { isBlockedAddress(it) }) {
            throw InvalidInputException("내부 네트워크 주소는 허용되지 않습니다.")
        }
