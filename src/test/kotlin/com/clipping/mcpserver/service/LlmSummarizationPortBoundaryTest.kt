...
class LlmSummarizationPortBoundaryTest {
    ...
    fun `services should depend on llm summarization port instead of concrete summarizer`() {
        ...
        serviceSources.forEach { fileName ->
            ...
            source shouldNotContain "import com.clipping.mcpserver.ai.ClippingSummarizer"
        }
    }
    ...
    fun `llm summarization port should not expose app models`() {
        val source = Files.readString(
            Paths.get("clipping-engine/src/main/kotlin/com/clipping/mcpserver/service/port/LlmSummarizationPort.kt")
        )
        ...
        source shouldNotContain "com.clipping.mcpserver.service.dto"
        source shouldNotContain "AiSummaryResponse"
        source shouldNotContain "AiDailySummaryResponse"
        source shouldNotContain "import com.clipping.mcpserver.service.dto.CompetitorTimelineItem"
    }

    @Test
    fun `clipping summarizer should be adapted instead of implementing the port directly`() {
        val summarizerSource = Files.readString(
            Paths.get("src/main/kotlin/com/clipping/mcpserver/ai/ClippingSummarizer.kt")
        )
        val adapterSource = Files.readString(
            Paths.get("src/main/kotlin/com/clipping/mcpserver/ai/LlmSummarizationAdapter.kt")
        )
