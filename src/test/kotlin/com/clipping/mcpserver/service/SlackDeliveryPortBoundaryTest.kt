...
class SlackDeliveryPortBoundaryTest {
    ...
    fun `slack delivery port should expose delivery operations only`() {
        val source = Files.readString(
            Paths.get("clipping-engine/src/main/kotlin/com/clipping/mcpserver/service/port/SlackDeliveryPort.kt")
        )
    ...
    fun `slack message sender contract should not depend on slack adapter types`() {
        val source = Files.readString(
            Paths.get("src/main/kotlin/com/clipping/mcpserver/service/SlackMessageSender.kt")
        )
    ...
    fun `batch and notification senders should depend on slack delivery port`() {
        ...
        val notificationSource = Files.readString(
            Paths.get(
                "clipping-notification/src/main/kotlin/com/clipping/mcpserver/service/notification/OperationsNotificationService.kt"
            )
        )
