...
class PipelinePortBoundaryTest {
    ...
    fun `pipeline orchestrators should depend on pipeline port instead of concrete clipping service`() {
        val serviceSourceRoot = Paths.get("src/main/kotlin/com/clipping/mcpserver/service")
    ...
    fun `pipeline port should live in engine module and expose independent pipeline DTOs`() {
        val appPortPath = Paths.get("src/main/kotlin/com/clipping/mcpserver/service/port/ClippingPipelinePort.kt")
        val enginePortPath =
            Paths.get("clipping-engine/src/main/kotlin/com/clipping/mcpserver/service/port/ClippingPipelinePort.kt")
        val portSource = Files.readString(enginePortPath)

        Files.exists(appPortPath) shouldBe false
        portSource shouldNotContain "com.clipping.mcpserver.model"
    ...
    fun `engine module should own pure external IO port contracts`() {
        val appPortRoot = Paths.get("src/main/kotlin/com/clipping/mcpserver/service/port")
        val enginePortRoot = Paths.get("clipping-engine/src/main/kotlin/com/clipping/mcpserver/service/port")
    ...
    fun `app use case ports should remain outside engine module`() {
        ...
        val appContractPortRoot =
            Paths.get("clipping-app-ports/src/main/kotlin/com/clipping/mcpserver/service/port")
        val enginePortRoot = Paths.get("clipping-engine/src/main/kotlin/com/clipping/mcpserver/service/port")
        ...
        digestDeliveryPortSource shouldNotContain "import com.clipping.mcpserver.service.port.PipelineDigestResult"
        ...
        digestDeliveryPortSource shouldNotContain "com.clipping.mcpserver.model"
    }

    @Test
    fun `app ports should not own API or pipeline DTOs`() {
        val appPortDtoRoot =
            Paths.get("clipping-app-ports/src/main/kotlin/com/clipping/mcpserver/service/dto")
        val apiModelDtoRoot =
            Paths.get("clipping-api-models/src/main/kotlin/com/clipping/mcpserver/service/dto/clipping")
        val pipelineModelDtoRoot =
            Paths.get("clipping-pipeline-models/src/main/kotlin/com/clipping/mcpserver/service/dto/pipeline")
    ...
    fun `persistence module should own entities and repositories`() {
        val appSourceRoot = Paths.get("src/main/kotlin/com/clipping/mcpserver")
        val persistenceSourceRoot = Paths.get("clipping-persistence/src/main/kotlin/com/clipping/mcpserver")
    ...
    fun `store SPI and implementations should be physically separated`() {
        val appSourceRoot = Paths.get("src/main/kotlin/com/clipping/mcpserver")
        val storeSpiRoot = Paths.get("clipping-store-spi/src/main/kotlin/com/clipping/mcpserver")
        val persistenceSourceRoot = Paths.get("clipping-persistence/src/main/kotlin/com/clipping/mcpserver")
    ...
    fun `shared service exceptions should not live in root app`() {
        val appErrorRoot = Paths.get("src/main/kotlin/com/clipping/mcpserver/error")
        val errorTypeRoot = Paths.get("clipping-error-types/src/main/kotlin/com/clipping/mcpserver/error")
    ...
    fun `clipping service should not implement pipeline port directly`() {
        val source = Files.readString(
            Paths.get("src/main/kotlin/com/clipping/mcpserver/service/ClippingService.kt")
        )
    ...
    fun `concrete clipping service should only be injected by adapter boundary classes`() {
        val sourceRoot = Paths.get("src/main/kotlin/com/clipping/mcpserver")
        val allowed = setOf(
                    ...
                    relative !in allowed &&
                        (
                            source.contains("private val clippingService: ClippingService") ||
                                source.contains("import com.clipping.mcpserver.service.ClippingService")
                            )
                }
    ...
    fun `app service digest package should be root app orchestration boundary`() {
        val digestSourceRoot = Paths.get("src/main/kotlin/com/clipping/mcpserver/service/digest")
        val digestApplicationSourceRoot =
            Paths.get("clipping-digest-application/src/main/kotlin/com/clipping/mcpserver/service/digest")
