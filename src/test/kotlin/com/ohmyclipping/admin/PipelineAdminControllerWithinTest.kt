...
class PipelineAdminControllerWithinTest {

    private val executionService = mockk<PipelineExecutionService>()
    private val controller = PipelineAdminController(executionService)
    ...
    @Nested
    inner class `listRuns within 파라미터` {

        @Test
        fun `within=1d 이면 오늘 KST 자정 Instant 로 서비스에 전달된다`() {
            // 오늘 KST 자정은 테스트 실행 동안 변하지 않는 고정 값이므로 엄격히 매칭한다.
            val expectedSince = todayKstMidnight()

            every {
                executionService.findRuns(
                    categoryId = null,
                    status = null,
                    since = any(),
                    offset = 0,
            ...
            controller.listRuns(
                ...
                personaId = null, within = "1d", page = 0, size = 30
            )
        ...
        fun `within=7d 이면 오늘 KST 자정의 6일 전 Instant 로 서비스에 전달된다 (오늘 포함 7일)`() {
            ...
            controller.listRuns(
                categoryId = null, status = null, from = null, to = null,
                personaId = null, within = "7d", page = 0, size = 30
            )
        ...
        fun `within 파라미터가 없으면 since=null 로 서비스에 전달된다`() {
            ...
            controller.listRuns(
                categoryId = null, status = null, from = null, to = null,
                personaId = null, within = null, page = 0, size = 30
            )
        ...
        fun `page가 음수이면 첫 페이지 offset으로 보정한다`() {
            ...
            val result = controller.listRuns(
                categoryId = null, status = null, from = null, to = null,
                personaId = null, within = null, page = -1, size = 30
            )
        ...
        fun `within=abc 이면 InvalidInputException 을 던진다`() {
            shouldThrow<InvalidInputException> {
                controller.listRuns(
                    categoryId = null, status = null, from = null, to = null,
                    personaId = null, within = "abc", page = 0, size = 30
                )
