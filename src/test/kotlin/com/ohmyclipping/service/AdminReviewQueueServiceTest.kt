...
class AdminReviewQueueServiceTest {
    ...
    inner class `listReviewItems perCategory 샘플링` {
        ...
        fun `perCategory 지정 시 카테고리별 top-N씩 뽑아 합집합을 반환한다`() {
            ...
            every { categoryStore.list() } returns listOf(
                com.ohmyclipping.model.Category(id = "cat-1", name = "카테고리1"),
                com.ohmyclipping.model.Category(id = "cat-2", name = "카테고리2"),
                com.ohmyclipping.model.Category(id = "cat-3", name = "카테고리3")
            )
        ...
        fun `perCategory가 0이면 InvalidInputException을 던진다`() {
            val ex = shouldThrow<com.ohmyclipping.error.InvalidInputException> {
    ...
    inner class `bulkApprove 메서드` {
        ...
        fun `ids가 101개이면 InvalidInputException을 던진다`() {
            ...
            val ex = io.kotest.assertions.throwables.shouldThrow<
                com.ohmyclipping.error.InvalidInputException
            > {
    ...
    inner class `getReviewStats 메서드` {
        ...
        fun `일치하는 항목이 있으면 정확도를 올바르게 계산한다`() {
            val rows = listOf(
                com.ohmyclipping.store.AccuracyRow("cat-1", "카테고리1", "INCLUDE", "INCLUDE", 3),
                com.ohmyclipping.store.AccuracyRow("cat-1", "카테고리1", "INCLUDE", "EXCLUDE", 1),
                com.ohmyclipping.store.AccuracyRow("cat-1", "카테고리1", "EXCLUDE", "EXCLUDE", 2)
            )
        ...
        fun `이전 기간에 데이터가 있으면 previousPeriodAccuracy가 계산된다`() {
            val currentRows = listOf(
                com.ohmyclipping.store.AccuracyRow("cat-1", "카테고리1", "INCLUDE", "INCLUDE", 2)
            )
            val prevRows = listOf(
                com.ohmyclipping.store.AccuracyRow("cat-1", "카테고리1", "INCLUDE", "INCLUDE", 4),
                com.ohmyclipping.store.AccuracyRow("cat-1", "카테고리1", "INCLUDE", "EXCLUDE", 1)
            )
